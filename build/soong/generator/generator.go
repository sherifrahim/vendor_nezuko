...
func init() {
	android.RegisterModuleType("nezuko_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = nezukoExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = nezukoExpandVariables(ctx, dep_file)
	...
	cmd := nezukoExpandVariables(ctx, String(g.properties.Cmd))
